...
using std::cout;
using std::endl;
//http://rosettacode.org/wiki/Knapsack_problem/Bounded#C.2B.2B 
class StopTimer
{
public:
...
void populateDataset(KnapsackTask& task)
{
    ...
    int capacity,items;
    int q,w,v;
    std:: cin>>capacity>>items;
    task.setMaxWeight( capacity);
    for ( int i = 0 ;i<items;i++)
    {
        std::cin>>q>>w>>v;

    task.add(Item("item",w,v,q));

    }
    /*task.add(Item("map",9,150,1));
    task.add(Item("map",13,35,1));
    task.add(Item("map",153,200,2));
    task.add(Item("sandwich",50,60,2));
    task.add(Item("glucose",15,60,2));
    task.add(Item("tin",68,45,3));
    task.add(Item("map",27,60,3));
...
for(int i = 1; i<=t;i++)
{
   ...
   // cout << "Iterations to solve: " << solution.iterations << endl;
    ...
    cout <<"Case #"<<i<<": ";
    for (unsigned k = 0; k < solution.n.size(); ++k)
    {
        if (const bool itemIsNotInKnapsack = !solution.n[k])
            continue;
        //cout << "  " << solution.n[i] << ' ' << task.getItems()[i].name << " ( item weight = " << task.getItems()[i].w << " )" << endl;
        int count = solution.n[k];
