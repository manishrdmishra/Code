...
int  execute_code(vector<string> code,int visited[][CODE_SIZE], int memory[][MEMORY_SIZE],int n ,int m, string& out)
{
    ...
    while ( true)
    {
        if(pcx < 0 || pcy < 0 || pcx >= m || pcy >= n || command_counter >= MAX_COMMAND)
        {
        ...
        if( visited[pcy][pcx] == false && loop_flag==false)
        {
            visited[pcy][pcx] = true;
        }
        else
        {
            if( loop_flag == false)
            ...
            else
            {
                ...
                else if( pcx == loop_start_x && pcy == loop_start_y )
                {
                    iteration++;
                    if( iteration ==2)
                    {

                        if((terminate_flag == false) || (previous_value - memory[y][x]== 0))
                        {
