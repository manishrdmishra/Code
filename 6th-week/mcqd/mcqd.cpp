...
#endif
  Maxclique(bool conn[10005][10005], const int, const float=0.025);
...
Maxclique::Maxclique (bool conn[][10005], const int sz, const float tt) : pk(0), level(1), Tlimit(tt), V(sz), Q(sz), QMAX(sz) {
  assert(conn!=0 && sz>0);
...
void read_dimacs( bool conn[][10005], int nodes, int edges) {
...
//  cout << "---------- Example 2: run max clique with improved coloring and dynamic sorting of vertices ----------------"<<endl;
  //start1 = time(NULL);
  //start2 = clock();
  Maxclique md(conn, nodes+1 , 0.001);  //(3rd parameter is optional - default is 0.025 - this heuristics parameter enables you to use dynamic resorting of vertices (time expensive)
